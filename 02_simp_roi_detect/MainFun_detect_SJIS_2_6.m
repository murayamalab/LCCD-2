function MainFun_detect_SJIS_2_6(in_path, out_path, options)
    mkdir(out_path);
    addpath(in_path);
    ...
    for FNum = 1:length(file_list)
        ...
        %tic;
        lumi_max_img_2d = std((ImageData-mean(ImageData,3))./mean(ImageData,3),[],3);
        %lumi_max_img_2d = imgaussfilt(lumi_max_img_2d,0.2);
        se = strel('disk',10);
        lumi_max_img_2d = imtophat(lumi_max_img_2d,se);
        sfl_size = 30;
        SP_fil = fspecial('gaussian',sfl_size, 0.5) - fspecial('average',sfl_size);
        lumi_max_img_2d = max(conv2(reshape(lumi_max_img_2d,Height,Width),SP_fil,'valid'),0);    
        ...
        BW5 = kubire_delete(BW); %１ピクセルで括れて連結している閉領域を２つに分ける(1 pixel重なっている場合、2つに分離)
        % Watershed
        ...
        L = labelmatrix(CC);
        BW6 = ismember(L, find([S.Area] >= options.pixels_range(1) & [S.Area] <= options.pixels_range(2)));
        RROI = bwlabel(BW6, 8);
